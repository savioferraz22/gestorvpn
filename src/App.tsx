            ...
            {view === "admin" && (
              ...
              >
                <div className="bg-gradient-to-r from-bg-surface to-bg-surface-hover p-6 flex justify-between items-center shrink-0 border-b border-border-base/50">
                  <div className="flex items-center">
                    <Settings className="w-6 h-6 text-text-base mr-3" />
                  ...
                  {!isAdminAuth ? (
                    <form onSubmit={(e) => {
                      ...
                      if (adminPass === (import.meta.env.VITE_ADMIN_PASSWORD || "Savio@2023")) {
                        ...
                        fetchAdminReports(adminReportPeriod);
                      } else {
                        setError("Senha incorreta");
                      }
                    }} className="space-y-4">
                      <div>
                        <label className="block text-sm font-medium text-text-base mb-1">Senha Administrativa</label>
                        <input
                          type="password"
                          value={adminPass}
                      ...
                      <button type="submit" className="w-full bg-bg-surface-hover hover:bg-bg-base text-text-base py-3 rounded-xl font-medium">
                        Entrar
                      </button>
                    </form>
                  ) : (
                    <div className="flex flex-col flex-1 overflow-hidden space-y-4">
                      <div className="flex gap-2.5 border-b border-border-base/50 pb-4 overflow-x-auto shrink-0 px-1 pt-1 no-scrollbar">
                        <button
                          onClick={() => setAdminTab("devices")}
                          className={`px-4 py-2.5 text-sm font-bold rounded-xl transition-all whitespace-nowrap active:scale-95 ${adminTab === "devices" ? "bg-text-base text-bg-base shadow-md" : "bg-bg-surface hover:bg-bg-surface-hover text-text-muted border border-border-base"}`}
                        >
                          Aparelhos
                        </button>
                        <button
                          onClick={() => setAdminTab("tickets")}
                          className={`px-4 py-2.5 text-sm font-bold rounded-xl transition-all whitespace-nowrap active:scale-95 ${adminTab === "tickets" ? "bg-text-base text-bg-base shadow-md" : "bg-bg-surface hover:bg-bg-surface-hover text-text-muted border border-border-base"}`}
                        >
                          Tickets
                        </button>
                        <button
                          onClick={() => setAdminTab("payments")}
                          className={`px-4 py-2.5 text-sm font-bold rounded-xl transition-all whitespace-nowrap active:scale-95 ${adminTab === "payments" ? "bg-text-base text-bg-base shadow-md" : "bg-bg-surface hover:bg-bg-surface-hover text-text-muted border border-border-base"}`}
                        >
                          Pagamentos
                        </button>
                        <button
                          onClick={() => setAdminTab("refunds")}
                          className={`px-4 py-2.5 text-sm font-bold rounded-xl transition-all whitespace-nowrap active:scale-95 ${adminTab === "refunds" ? "bg-text-base text-bg-base shadow-md" : "bg-bg-surface hover:bg-bg-surface-hover text-text-muted border border-border-base"}`}
                        >
                          Reembolsos
                        </button>
                        <button
                          onClick={() => setAdminTab("change_requests")}
                          className={`px-4 py-2.5 text-sm font-bold rounded-xl transition-all whitespace-nowrap active:scale-95 ${adminTab === "change_requests" ? "bg-text-base text-bg-base shadow-md" : "bg-bg-surface hover:bg-bg-surface-hover text-text-muted border border-border-base"}`}
                        >
                          Alterações
                        </button>
                        <button
                          onClick={() => setAdminTab("reports")}
                          className={`px-4 py-2.5 text-sm font-bold rounded-xl transition-all whitespace-nowrap active:scale-95 ${adminTab === "reports" ? "bg-text-base text-bg-base shadow-md" : "bg-bg-surface hover:bg-bg-surface-hover text-text-muted border border-border-base"}`}
                        >
                          Relatórios
                        </button>
                      </div>
                      ...
                      {adminTab === "devices" ? (
                        <div className="flex flex-col flex-1 overflow-hidden space-y-4">
                          <div className="flex justify-between items-center shrink-0 mt-2">
                            <h2 className="font-bold text-text-base">Aparelhos Registrados ({devices.length})</h2>
                            <button
                              onClick={clearAllDevices}
                              className="text-xs bg-red-50 hover:bg-red-100 border border-red-100 text-red-600 px-3 py-2 rounded-xl font-bold transition-colors flex items-center shadow-sm active:scale-95 uppercase tracking-wider"
                            >
                          ...
                          <div className="flex-1 overflow-y-auto space-y-3 pr-2 scrollbar-hide">
                            {devices.length === 0 ? (
                              <div className="bg-bg-surface/50 border border-border-base/50 p-6 rounded-2xl text-center">
                                <p className="text-sm font-medium text-text-muted">Nenhum aparelho registrado.</p>
                              </div>
                            ) : (
                              devices.map((d: any) => (
                                <div key={d.device_id} className="bg-bg-surface border border-border-base/50 p-4 rounded-2xl flex justify-between items-center shadow-sm">
                                  <div className="overflow-hidden">
                                    <p className="text-sm font-bold text-text-base truncate">Usuário: {d.username}</p>
                                    <p className="text-[11px] font-mono font-medium text-text-muted truncate mt-1 bg-bg-surface-hover inline-block px-1.5 py-0.5 rounded border border-border-base/50" title={d.device_id}>ID: {(d.device_id || '').substring(0, 16)}...</p>
                                  </div>
                                  <button
                                    onClick={() => {
                                      confirmAction("Remover Aparelho", "Remover este aparelho?", async () => {
                                        await fetch(`/api/admin/devices/${d.device_id}`, { method: "DELETE" });
                                        fetchDevices();
                                      });
                                    }}
                                    className="text-red-600 bg-red-50 hover:bg-red-100 border border-red-100 p-2.5 rounded-xl transition-colors shadow-sm active:scale-95"
                                  >
                                    <Trash2 className="w-4 h-4" />
                                  </button>
                                </div>
                        ...
                        <div className="flex flex-col flex-1 overflow-hidden space-y-3">
                          <div className="flex gap-2 shrink-0 mt-2">
                            <div className="relative flex-1 shadow-sm rounded-xl">
                              <Search className="w-4 h-4 absolute left-3.5 top-1/2 transform -translate-y-1/2 text-text-muted" />
                              <input
                                type="text"
                                placeholder="Buscar usuário..."
                                value={adminTicketSearchUser}
                                onChange={e => setAdminTicketSearchUser(e.target.value)}
                                className="w-full pl-10 pr-4 py-3 rounded-xl border border-border-base text-sm outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500 bg-bg-surface transition-all font-medium"
                              />
                            </div>
                            <select
                              value={adminTicketFilterStatus}
                              onChange={e => setAdminTicketFilterStatus(e.target.value)}
                              className="px-4 py-3 rounded-xl border border-border-base text-sm outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500 bg-bg-surface shadow-sm font-bold cursor-pointer transition-all"
                            >
                          ...
                          <div className="flex-1 overflow-y-auto space-y-3 pr-2 scrollbar-hide pt-1">
                            {allTickets
                              .filter(t => adminTicketFilterStatus === "all" || t.status === adminTicketFilterStatus)
                              .filter(t => t.username.toLowerCase().includes(adminTicketSearchUser.toLowerCase()))
                              .length === 0 ? (
                              <div className="bg-bg-surface/50 border border-border-base/50 p-6 rounded-2xl text-center">
                                ...
                                .map(t => (
                                  <div
                                    ...
                                    onClick={() => {
                                      setCurrentTicket(t);
                                      fetchMessages(t.id);
                                      fetchAdminTicketUserDetails(t.username);
                                      setView("admin_ticket_detail");
                                    }}
                                  ...
                                  >
                                    <div className="flex justify-between items-start mb-2">
                                      ...
                                      <span className={`text-[10px] uppercase tracking-wider px-2.5 py-1 rounded-lg flex-shrink-0 font-bold border ${t.status === 'open' ? 'bg-amber-50 text-amber-700 border-amber-200' :
                        ...
                        <div className="flex flex-col flex-1 overflow-hidden space-y-4">
                          <div className="flex justify-between items-center shrink-0 mt-2">
                            <h2 className="font-bold text-text-base">Histórico de Pagamentos</h2>
                            <button
                              onClick={fetchAdminPayments}
                              className="text-xs bg-primary-50 border border-primary-100 hover:bg-primary-100 text-primary-700 px-3 py-2 rounded-xl font-bold transition-colors flex items-center shadow-sm active:scale-95 uppercase tracking-wider"
                            >
                          ...
                          <div className="grid grid-cols-3 gap-3 mb-2 shrink-0">
                            <div className="bg-bg-surface border border-border-base/50 p-4 rounded-2xl text-center shadow-sm">
                              <p className="text-[10px] uppercase font-bold text-text-muted mb-1 tracking-wider">Aprovados</p>
                              <p className="text-2xl font-bold text-green-600 leading-none">{adminPayments.filter(p => p.status === 'approved').length}</p>
                            </div>
                            <div className="bg-bg-surface border border-border-base/50 p-4 rounded-2xl text-center shadow-sm">
                              <p className="text-[10px] uppercase font-bold text-text-muted mb-1 tracking-wider">Pendentes</p>
                              <p className="text-2xl font-bold text-amber-600 leading-none">{adminPayments.filter(p => p.status === 'pending').length}</p>
                            </div>
                            <div className="bg-bg-surface border border-border-base/50 p-4 rounded-2xl text-center shadow-sm">
                              <p className="text-[10px] uppercase font-bold text-text-muted mb-1 tracking-wider">Receita Total</p>
                              <p className="text-xl font-bold text-primary-600 leading-none mt-1">
                                R$ {adminPayments.reduce((acc, p) => {
                                  if (p.status === 'approved' && p.metadata) {
                                    try {
                                      const meta = JSON.parse(p.metadata);
                                      if (meta.amount) return acc + Number(meta.amount);
                                    } catch (e) { }
                                  }
                                  return acc;
                                }, 0).toFixed(2).replace('.', ',')}
                                  ...
                                  <div key={p.id} className="bg-bg-surface border border-border-base/50 p-4 rounded-2xl flex flex-col gap-3 shadow-sm hover:shadow-md transition-shadow">
                                    <div className="flex justify-between items-start">
                                      <div>
                                        ...
                                        <p className="text-[11px] font-mono bg-bg-surface-hover px-1.5 py-0.5 rounded text-text-muted border border-border-base/50">ID: {p.id.substring(0, 8)}...</p>
                                      </div>
                                      <div className="flex flex-col items-end gap-1.5">
                                        <span className={`text-[10px] uppercase font-bold tracking-wider px-2.5 py-1 rounded-lg border ${p.status === 'approved' ? 'bg-green-50 text-green-700 border-green-200' :
                                    ...
                                    <div className="flex justify-between items-center text-[11px] text-text-muted border-t border-border-base/50 pt-2.5 font-medium uppercase tracking-wide">
                                      <span className="bg-bg-surface-hover px-2 py-1 rounded-md">{p.type === 'new_device' ? 'Novo Aparelho' : 'Renovação'}</span>
                        ...
                        <div className="flex flex-col flex-1 overflow-hidden space-y-4">
                          <div className="flex justify-between items-center shrink-0 mt-2">
                            <h2 className="font-bold text-text-base">Solicitações de Reembolso</h2>
                            <button
                              onClick={fetchAdminRefunds}
                              className="text-xs bg-primary-50 border border-primary-100 hover:bg-primary-100 text-primary-700 px-3 py-2 rounded-xl font-bold transition-colors flex items-center shadow-sm active:scale-95 uppercase tracking-wider"
                            >
                              <RefreshCw className="w-3.5 h-3.5 mr-1.5" />
                              Atualizar
                                ...
                                <div key={r.id} className="bg-bg-surface border border-border-base/50 p-5 rounded-2xl flex flex-col gap-4 shadow-sm hover:shadow-md transition-shadow">
                                  <div className="flex justify-between items-start">
                                    <div>
                                      ...
                                      <p className="text-[11px] font-medium text-text-muted bg-bg-surface-hover px-1.5 py-0.5 rounded inline-block border border-border-base/50">{formatDate(r.created_at)}</p>
                                    </div>
                                    <span className={`text-[10px] uppercase font-bold tracking-wider px-2.5 py-1 rounded-lg border flex-shrink-0 ${r.status === 'realizado' ? 'bg-green-50 text-green-700 border-green-200' :
                                  ...
                                  <div className="bg-bg-base/50 p-3.5 rounded-xl text-sm border border-border-base/50">
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                      <p className="font-bold text-text-base text-xs bg-white/50 p-1.5 rounded border border-black/5"><span className="text-text-muted mr-1">Tipo:</span> {r.pix_type}</p>
                                      <p className="font-bold text-text-base text-xs break-all bg-white/50 p-1.5 rounded border border-black/5"><span className="text-text-muted mr-1">Chave:</span> {r.pix_key}</p>
                                    </div>
                                    {r.refunded_at && (
                                      <p className="font-bold text-green-600 mt-3 text-xs flex items-center"><RefreshCw className="w-3 h-3 mr-1"/>Realizado em {formatDate(r.refunded_at)}</p>
                                    )}
                                  </div>
                                  {r.status === 'aguardando' && (
                                    <div className="flex flex-col gap-3 pt-3 border-t border-border-base/50">
                                      {approvingRefundId === r.id ? (
                                        <div className="flex flex-col gap-3 bg-bg-base/50 p-4 rounded-xl border border-border-base/50">
                                          ...
                                          <input
                                            ...
                                            className="w-full px-4 py-3 rounded-xl border border-border-base focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500 outline-none text-sm bg-bg-surface text-text-base font-medium transition-all"
                                          />
                                          <div className="flex gap-2.5 mt-1">
                                            <button
                                              onClick={async () => {
                                                if (!refundDateTime) {
                                                  showAlertDialog("Selecione a data e hora do reembolso.");
                                                  return;
                                                }
                                                await fetch(`/api/admin/refunds/${r.id}/approve`, {
                                                  method: "POST",
                                                  headers: { "Content-Type": "application/json" },
                                                  body: JSON.stringify({ refundedAt: new Date(refundDateTime).toISOString() })
                                                });
                                                setApprovingRefundId(null);
                                                fetchAdminRefunds();
                                              }}
                                              className="flex-1 bg-green-500 hover:bg-green-600 text-white py-2.5 rounded-xl text-sm font-bold transition-all shadow-sm active:scale-95"
                                            >
                                              Confirmar
                                            </button>
                                            <button
                                        ...
                                        <div className="flex gap-2.5">
                                          ...
                                          <button
                                            onClick={() => {
                                              confirmAction("Rejeitar Reembolso", "Rejeitar esta solicitação?", async () => {
                                                await fetch(`/api/admin/refunds/${r.id}/reject`, { method: "POST" });
                                                fetchAdminRefunds();
                                              });
                                            }}
                      ...
                      ) : adminTab === "change_requests" ? (
                        <div className="flex flex-col flex-1 overflow-hidden space-y-4">
                          <div className="flex justify-between items-center shrink-0 mt-2">
                            <h2 className="font-bold text-text-base">Solicitações de Alteração</h2>
                            <button
                              onClick={fetchAdminChangeRequests}
                              className="text-xs bg-primary-50 border border-primary-100 hover:bg-primary-100 text-primary-700 px-3 py-2 rounded-xl font-bold transition-colors flex items-center shadow-sm active:scale-95 uppercase tracking-wider"
                            >
                              <RefreshCw className="w-3.5 h-3.5 mr-1.5" />
                              Atualizar
                          ...
                          <div className="flex-1 overflow-y-auto space-y-3 pr-2 scrollbar-hide pt-1">
                            {adminChangeRequests.length === 0 ? (
                              <div className="bg-bg-surface/50 border border-border-base/50 p-6 rounded-2xl text-center">
                                <p className="text-sm font-medium text-text-muted">Nenhuma solicitação de alteração.</p>
                              </div>
                            ) : (
                              adminChangeRequests.map((r: any) => (
                                <div key={r.id} className="bg-bg-surface border border-border-base/50 p-5 rounded-2xl flex flex-col gap-4 shadow-sm hover:shadow-md transition-shadow">
                                  <div className="flex justify-between items-start">
                                    <div>
                                      <p className="text-sm font-bold text-text-base mb-1">{r.username}</p>
                                      <div className="flex gap-2 items-center">
                                        <span className="text-[11px] font-medium text-text-muted bg-bg-surface-hover px-1.5 py-0.5 rounded border border-border-base/50">{formatDate(r.created_at)}</span>
                                        <p className="text-[10px] uppercase font-bold text-primary-600 bg-primary-50 px-2 py-0.5 rounded border border-primary-100">
                                          {r.type === 'date' ? 'Vencimento' : r.type === 'username' ? 'Usuário' : r.type === 'uuid' ? 'UUID' : 'Senha'}
                                        </p>
                                      </div>
                                    </div>
                                    <span className={`text-[10px] uppercase font-bold tracking-wider px-2.5 py-1 rounded-lg border flex-shrink-0 ${r.status === 'aprovado' ? 'bg-green-50 text-green-700 border-green-200' :
                                      r.status === 'aguardando' ? 'bg-amber-50 text-amber-700 border-amber-200' :
                                        'bg-red-50 text-red-700 border-red-200'
                                      }`}>
                                  ...
                                  {r.status === 'aguardando' && (
                                    <div className="flex gap-2.5 pt-3 border-t border-border-base/50">
                                      <button
                                        onClick={() => {
                                          let approvedValue = r.requested_value;
                                          if (r.type === 'username') {
                                            const val = prompt("Confirme o novo usuário aprovado:", r.requested_value);
                                            if (val === null) return;
                                            approvedValue = val;
                                          }
                                          confirmAction("Aprovar Alteração", `Aprovar esta alteração? Lembre-se de alterar no painel VPN também.`, async () => {
                                            await fetch(`/api/admin/change-requests/${r.id}/approve`, {
                                              method: "POST",
                                              headers: { "Content-Type": "application/json" },
                                      ...
                                      <button
                                        onClick={() => {
                                          confirmAction("Rejeitar Alteração", "Rejeitar esta solicitação?", async () => {
                                            await fetch(`/api/admin/change-requests/${r.id}/reject`, { method: "POST" });
                                            fetchAdminChangeRequests();
                                          });
                                        }}
                                        className="flex-[0.4] bg-red-50 hover:bg-red-100 border border-red-200 text-red-700 py-2.5 rounded-xl text-sm font-bold transition-all shadow-sm active:scale-95"
                        ...
                        <div className="flex flex-col flex-1 overflow-hidden space-y-4">
                          <div className="flex justify-between items-center shrink-0 mt-2">
                            <h2 className="font-bold text-text-base">Relatórios e Métricas</h2>
                            <div className="flex items-center gap-2">
                              <select
                                value={adminReportPeriod}
                                onChange={(e) => {
                                  const p = Number(e.target.value);
                                  setAdminReportPeriod(p);
                                  fetchAdminReports(p);
                                }}
                                className="text-xs bg-bg-surface border border-border-base/50 rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-primary-500/50 font-bold cursor-pointer transition-all shadow-sm"
                              >
                              ...
                              <button
                                onClick={() => fetchAdminReports(adminReportPeriod)}
                                className="text-xs bg-primary-50 border border-primary-100 hover:bg-primary-100 text-primary-700 p-2 rounded-xl font-bold transition-colors shadow-sm active:scale-95"
                              >
                                <RefreshCw className="w-4 h-4" />
                              </button>
                            </div>
                          ...
                          {!adminReports ? (
                            <p className="text-sm text-text-muted text-center py-8">Carregando relatórios...</p>
                          ) : (
                            <div className="flex-1 overflow-y-auto space-y-6 pr-2 scrollbar-hide pt-1">
                              {/* KPIs */}
                              <div className="grid grid-cols-2 gap-3">
                                <div className="bg-bg-surface border border-border-base/50 p-4 rounded-2xl shadow-sm">
                                  ...
                                  <p className="text-2xl font-bold text-primary-600 leading-none">
                                    R$ {Number(adminReports.totalRevenue).toFixed(2).replace('.', ',')}
                                  </p>
                                </div>
                                <div className="bg-bg-surface border border-border-base/50 p-4 rounded-2xl shadow-sm">
                                  <p className="text-[10px] uppercase font-bold tracking-wider text-text-muted mb-1">Vendas Concluídas</p>
                                  <p className="text-2xl font-bold text-text-base leading-none">{adminReports.totalSales}</p>
                                </div>
                                <div className="bg-bg-surface border border-border-base/50 p-4 rounded-2xl shadow-sm">
                                  <p className="text-[10px] uppercase font-bold tracking-wider text-text-muted mb-1">Testes Gerados</p>
                                  <p className="text-2xl font-bold text-text-base leading-none">{adminReports.totalTests}</p>
                                </div>
                                <div className="bg-bg-surface border border-border-base/50 p-4 rounded-2xl shadow-sm">
                                  <p className="text-[10px] uppercase font-bold tracking-wider text-text-muted mb-1">Taxa de Conversão</p>
                                  <p className="text-2xl font-bold text-green-600 leading-none">{adminReports.conversionRate}%</p>
                                </div>
                              </div>
                              ...
                              <div>
                                ...
                                {adminReports.salesHistory.length === 0 ? (
                                  <div className="bg-bg-surface/50 border border-border-base/50 p-6 rounded-2xl text-center">
                                  ...
                                  <div className="bg-bg-surface border border-border-base/50 rounded-2xl overflow-hidden shadow-sm">
                                    <table className="w-full text-sm text-left">
                                      ...
                                      <tbody>
                                        {adminReports.salesHistory.map((s: any) => (
                                          <tr key={s.date} className="border-b border-border-base/50 last:border-0 hover:bg-bg-surface-hover transition-colors">
                              ...
                              <div>
                                ...
                                {adminReports.testsHistory.length === 0 ? (
                                  <div className="bg-bg-surface/50 border border-border-base/50 p-6 rounded-2xl text-center">
                                    <p className="text-sm font-medium text-text-muted">Nenhum teste gerado no período.</p>
                                  </div>
                                ) : (
                                  <div className="bg-bg-surface border border-border-base/50 rounded-2xl overflow-hidden shadow-sm">
                                    <table className="w-full text-sm text-left">
